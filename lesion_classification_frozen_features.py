...
def extract_features_and_labels(
    ...
    for (
        ...
        if "vit" in args.arch:
            intermediate_output = encoder.get_intermediate_layers(image, n_last_blocks)
...
def train_mil(gpu, result_queue, fold_queue, args):
    ...
    while fold_queue.qsize() > 0:
        ...
        # otherwise, we check if the architecture is in torchvision models
            ...
            encoder = efficientformer.__dict__[args.arch]()
        ...
        try:
            (train_features, train_targets, _) = torch.load(train_serialize_path)
        except (RuntimeError, FileNotFoundError):
            (train_features, train_targets, _) = extract_and_save(
                encoder,
                fit_dataset,
                args.num_workers,
                args,
        ...
        try:
            ...
            (test_features, test_targets, test_images,) = extract_and_save(
                encoder,
                test_dataset,
                0,
                args,
...
def extract_and_save(model, dataset, num_workers, args, path, return_images=False):
    ...
    (features, targets, images,) = extract_features_and_labels(
        model,
        loader,
        multiscale=False,
...
def train(model, optimizer, features, targets, epoch, batch_size):
    logger = logging.getLogger()
    model.train()
    metric_logger = utils.MetricLogger(delimiter="  ")
    ...
    # Merge last partial batch with previous batch
    ...
    for lower, upper in metric_logger.log_every(list(zip(lowers, uppers)), 19, header):
        ...
        batch_targets = targets[lower:upper].cuda(non_blocking=True)

        # forward
        output = model.calc_head(batch_features)
...
def validate_network(
    ...
    (acc,) = utils.accuracy(output, targets, topk=(1,))

    metric_logger.update(loss=loss.item())
    metric_logger.meters["acc"].update(acc.item())
...
if __name__ == "__main__":
    ...
    # TODO ditch representation memoization in favor of randomly offset training patches
    if not os.path.isfile(os.path.join(args.features_dir, "datasets.pth.tar")):
        ...
        train_transform = pth_transforms.Compose(
            [
                ...
                pth_transforms.Normalize(CHIMEC_MEAN, CHIMEC_STD),
                GridTile(
                    tile_size=args.tile_size, overlap=args.overlap, max_frac_black=0.96
                ),
            ]
        )
